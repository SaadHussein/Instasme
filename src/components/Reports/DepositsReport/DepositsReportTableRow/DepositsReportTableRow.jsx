		...
		<tr>
			...
			{columnsVisible.map(
				(column) =>
					...
					column.visible && (
						<td className="text-gray-800 text-hover-primary fw-bold">
							{count === 0 ? (
								<CountUp start={0} end={totalDeposits} duration={3} />
							) : (
								totalDeposits
							)}
						</td>
				...
				(column) =>
					...
					column.visible && (
						<td className="text-end text-primary fw-bold">
							{count === 0 ? (
								<CountUp start={0} end={totalSales} duration={3} />
							) : (
								totalSales
							)}
						</td>
			...
			{/* {columnsVisible.map(
				(column) =>
					...
					column.visible && (
						<td className="text-end text-gray-800 fw-bold">{category}</td>
					)
			)}
			{columnsVisible.map(
				(column) =>
					...
					column.visible && (
						<td className="text-end text-gray-800 fw-bold">{currentStock}</td>
					)
			)} */}
			{/* {columnsVisible.map(
				(column) =>
...
export default DepositsReportTableRow;
