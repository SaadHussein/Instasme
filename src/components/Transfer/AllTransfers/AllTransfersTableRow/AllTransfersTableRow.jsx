		...
		<tr>
			...
			{columnsVisible.map(
				(column) =>
					...
					column.visible && (
						<td className="text-end fw-bold text-gray-600">{toWarehouse}</td>
					)
			)}
			{columnsVisible.map(
				(column) =>
					...
					column.visible && (
						<td className="text-end text-primary fw-bold">
							$
							{count === 0 ? (
								<CountUp start={0} end={items} duration={3} />
							) : (
								items
							)}
						</td>
			...
			{columnsVisible.map(
				(column) =>
					...
					column.visible && (
						<td className="text-end text-primary fw-bold">
							{status === "Completed" ? <Completed text={status} /> : ""}
						</td>
					)
			)}
...
export default AllTransfersTableRow;
