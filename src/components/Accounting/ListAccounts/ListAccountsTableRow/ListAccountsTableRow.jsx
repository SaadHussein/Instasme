		...
		<tr>
				...
				(column) =>
					...
					column.visible && (
						<td className="text-end text-primary fw-bold">
							$
							{count === 0 ? (
								<CountUp start={0} end={balance} duration={3} />
							) : (
								balance
							)}
						</td>
			...
			{columnsVisible.map(
				(column) =>
					column.label === "Notes" &&
					column.visible && (
						<td className="text-end text-gray-600 text-hover-primary fw-bold">
							{notes}
						</td>
					)
			)}
...
export default ListAccountsTableRow;
