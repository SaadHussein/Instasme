...
const CustomersTable = ({ columnsVisible, customersTableItems }) => {
	const [customers, setCustomers] = useState([...customersTableItems]);
	...
	const sortUponItem = () => {
		if (order === "no-order") {
			...
			setOrder("no-order");
			setSortItem("");
			setCustomers([...customersTableItems]);
		}
	};
	return (
		<div
				...
				<table className="table align-middle table-row-bordered table-row-solid gy-4 gs-9">
					<thead className="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
						<tr>
							...
							{columnsVisible.map(
								(column) =>
									column.label === "Total Sale Due" &&
									column.visible && (
										<th
											className="text-end min-w-70px cursor-pointer text-hover-primary pe-0"
											onClick={() => {
												setSortItem("totalSaleDue");
												sortUponItem();
											}}
										>
											<p
											...
											>
												...
												>
													<MdOutlineKeyboardArrowUp
														size={18}
														color={`${
															sortItem === "totalSaleDue" &&
															order === "ascending"
																? "#1b84ff"
																: sortItem !== "totalSaleDue" || sortItem === ""
																? "#99A1B7"
																: ""
														}`}
														style={{
							...
							{columnsVisible.map(
								(column) =>
									column.label === "Total Sale Return Due" &&
									column.visible && (
										<th
											className="text-end min-w-70px cursor-pointer text-hover-primary pe-0"
											onClick={() => {
												setSortItem("totalSaleReturnDue");
												sortUponItem();
											}}
										>
											<p
					...
					<tbody className="fs-6 fw-semibold text-gray-600">
						{customers.map((item, index) => (
							<CustomersTableRow
								...
								totalSaleDue={item.totalSaleDue}
								totalSaleReturnDue={item.totalSaleReturnDue}
								index={index}
							/>
						))}
					</tbody>
